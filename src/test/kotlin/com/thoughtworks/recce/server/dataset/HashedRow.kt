...
data class HashedRow(val migrationKey: String, val hashedValue: String) {
    companion object {
        val migrationKeyColumnName: String = "MigrationKey"
    }
}
...
fun toHashedRow(row: Row, meta: RowMetadata): HashedRow {
    ...
    val migrationKey = row.get(HashedRow.migrationKeyColumnName, String::class.java)
        ?: throw IllegalArgumentException("Result rows do not have String column called [${HashedRow.migrationKeyColumnName}]!")

    val hash = Hashing.sha256().newHasher()
    meta.columnNames
        .filter { !it.equals(HashedRow.migrationKeyColumnName) }
        .forEach {
